...
export const AnimatedButton: React.FC<AnimatedButtonProps> = ({
  ...
  // Size variants
  const sizeStyles = {
    sm: "h-[40px] sm:h-[44px] text-[14px] sm:text-[15px] px-2",
    md: "h-[30px] w-[50] sm:h-[56px] text-[16px] sm:text-[18px] px-3 lg:w-[90px]",
    lg: "h-[60px] sm:h-[68px] text-[18px] sm:text-[20px] px-4",
  };
  ...
  const variantStylesMap = {
    start: {
      normal: {
        ...
        bgColor: "bg-gradient-to-tr animated-background from-red-500 to-orange-300",
    ...
    docs: {
      normal: {
        ...
        shadow: "shadow-theme-sm",
        border: "border-[4px] border-theme-border-light",
        pattern:
  ...
  return (
    <button
      ...
      className={`
        ...
        rounded-[12px] font-[500]
        transition-all duration-[var(--border-theme-border-light)]
        ${disabled ? 'opacity-60 cursor-not-allowed' : 'hover-bubbly focus:scale-[1.02]'}
        focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-opacity-50
        ${variant === "docs" ? "bg-theme-primary hover:bg-theme-bg-secondary hover:opacity-[10%] focus:ring-theme-primary" 
          : "hover:brightness-120 focus:brightness-120 focus:ring-white"}
        ${className}
      `}
      onClick={onClick}
    >
